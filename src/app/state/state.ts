...
import { distinctUntilChanged, map, mergeAll, observeOn, pluck, publishReplay, scan, tap } from 'rxjs/operators';
import { stateful } from './operators';

export const pipeFromArray = <T, R>(
...
export class State<T> implements OnDestroy {
    ...
    protected readonly state$ = merge(
    ...
    private stateAccumulator(acc: T, command: Partial<T>): T {
        ...
        // return simpleMergeAccumulator(acc, slices);
        return { ...a, ...c } as T;
    }
    ...
    connect<A extends keyof T>(strOrObs: any, obs?: any): void {
        if (typeof strOrObs === 'string') {
            this.stateObservables.next(obs.pipe(
                map(s => ({ [strOrObs as A]: s })),
    ...
    select(...opOrMapFn: any[]): Observable<any> {
        ...
        throw new Error(
            'Wrong params passed! Passed param:' + JSON.stringify(opOrMapFn)
        );
    }
